...
if [ ! -e ./ec2_instance/instance-id.txt ]; then
    ...
    INSTANCE_ID=$(aws ec2 run-instances  --user-data file://./provisiononing/${INSTANCE_INIT_SCRIPT} --image-id ${AMI_IMAGE_ID} --security-group-ids ${SECURITY_GROUP_ID} --count 1 --instance-type t2.micro --key-name ${SECURITY_GROUP_NAME} --query 'Instances[0].InstanceId'  --output=text)
